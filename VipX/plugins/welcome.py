...
async def join_watcher(_, message):    
    ...
    for members in message.new_chat_members:
            ...
            msg = (
                f"**🌷Hᴇʏ Bᴀᴇ {message.from_user.mention} Wᴇʟᴄᴏᴍᴇ Tᴏ {message.chat.title} 🦋🙈 **\n\n"
                f"**✍️Yᴏᴜʀ UsᴇʀNᴀᴍᴇ:** @{message.from_user.username}\n➖➖➖➖➖➖➖➖➖➖➖\n"
                f"**💖Yᴏᴜʀ Iᴅ:** {message.from_user.id}\n➖➖➖➖➖➖➖➖➖➖➖\n"
               # f"**💖𝐔ʀ 𝐈d:** {message.from_user.id}\n➖➖➖➖➖➖➖➖➖➖➖\n"
               # f"**✍️𝐔ʀ 𝐔.𝐍:** @{message.from_user.username}\n➖➖➖➖➖➖➖➖➖➖➖\n"
                f"**Yᴏᴜ Hᴀᴠᴇ Cᴏᴍᴘʟᴇᴛᴇᴅ {count} Mᴇᴍʙᴇʀ𝐬🎉**"
            )
            await app.send_photo(message.chat.id, photo=random.choice(photo), caption=msg, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"Wᴇʟᴄᴏᴍᴇ Tᴏ Nᴇᴡ Cʜᴀᴛ 💝", url=f"https://t.me/{app.username}?startgroup=true")]
         ]))
